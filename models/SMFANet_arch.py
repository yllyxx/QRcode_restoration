...
class SMFA(nn.Module):
    def __init__(self, dim=36):
        ...
        # self.dw_conv = nn.Conv2d(dim,dim,3,1,1,groups=dim)
        self.conv_1 = nn.Conv2d(dim,dim, kernel_size=1, padding=0, stride=1, groups=1,
                               bias=False)
        # self.gelu = nn.GELU()
        self.down_scale = 4
    ...
    def forward(self, f):
        ...
        x = f
        # y, x = self.linear_0(f).chunk(2, dim=1)
        x_s = self.conv_1(F.adaptive_max_pool2d(x, (h // self.down_scale, w // self.down_scale)))
        x_v = torch.var(x, dim=(-2,-1), keepdim=True)
        # x_l = x * F.interpolate(self.linear_1(x_s * self.alpha + x_v * self.belt), size=(h,w), mode='nearest')
        x_l =  F.interpolate(
