...
class duochiduconv(nn.Module):
    def __init__(self, n_feats, k=2, squeeze_factor=15):
        ...
        # Define convolutions with different kernel sizes without groups
        self.conv3x3 = nn.Conv2d(n_feats // 4, n_feats // 4, 3, 1, 3 // 2)
        self.conv5x5 = nn.Conv2d(n_feats // 4, n_feats // 4, 5, 1, 5 // 2)
        self.conv7x7 = nn.Conv2d(n_feats // 4, n_feats // 4, 7, 1, 7 // 2)
        self.conv9x9 = nn.Conv2d(n_feats // 4, n_feats // 4, 9, 1, 9 // 2)
    ...
    def forward(self, x, pre_attn=None, RAA=None):
        ...
        # Split channels into four parts
        a_1, a_2, a_3, a_4 = torch.chunk(x, 4, dim=1)

        # Apply different convolutions
        x = torch.cat([self.conv3x3(a_1), self.conv5x5(a_2), self.conv7x7(a_3), self.conv9x9(a_4)], dim=1)
