...
#     # def __init__(self, in_channel, out_channel, filter=True):
...
#         super().__init__()
#         self.k = k
#         self.global_att = GlobalPoolStripAttention(k//4)
#         self.local_att_3 = LocalPoolStripAttention(k // 4, kernel=3)
#         self.local_att_5 = LocalPoolStripAttention(k // 4, kernel=5)
#         self.local_att_7 = LocalPoolStripAttention(k // 4, kernel=7)
#         self.conv = nn.Conv2d(k, k, 1)
#
#     def forward(self, x):
#         k = self.k
#         split = torch.split(x, k // 4, dim=1)
#         # global_out = self.global_att(x)
#         # local_3_out = self.local_att_3(x)
#         # local_5_out = self.local_att_5(x)
#         global_out = self.global_att(split[0])
#         local_3_out = self.local_att_3(split[1])
#         local_5_out = self.local_att_5(split[2])
#         local_7_out = self.local_att_7(split[3])
#         # out = global_out + local_3_out + local_5_out + local_7_out
#         out = torch.cat([global_out,local_3_out,local_5_out,local_7_out], dim=1)
...
class SpecAtte(nn.Module):
    def __init__(self, k) -> None:
        ...
        self.local_att_3 = LocalPoolStripAttention(k, kernel=3)
        self.local_att_5 = LocalPoolStripAttention(k, kernel=5)
        self.conv = nn.Conv2d(k, k, 1)
